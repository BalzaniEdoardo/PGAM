...
class weights_and_data(object):
    ...
    def alpha(self, mu):
        if not self.fisher_scoring:
            FLOAT_EPS = np.finfo(float).eps
            dy = self.y - mu
            corr = self.family.variance.deriv(mu) / self.family.variance(
                mu
            ) + self.family.link.deriv2(mu) / self.family.link.deriv(mu)
            alpha = 1.0 + dy * np.clip(corr, FLOAT_EPS, np.inf)
        else:
            alpha = 1.0
        return alpha
...
def gcv_grad_comp(
    rho, X, Q, R, endog, sm_handler, var_list, return_par="gcv", gamma=1.0
):
    """
    ...
    gcv_grad = (n_obs / delta**2) * alpha_grad - (
        2 * n_obs * alpha / delta**3
    ) * delta_grad
        ...
        for k in range(rho.shape[0]):
            ...
            if return_par == "A":
                A_hes[j, k] = (
                    ...
                    * Dinv
                    * V_T
                    * (Sk * V_T.T * D2inv * V_T * Sj + Sj * V_T.T * D2inv * V_T * Sk)
                    * V_T.T
                    * Dinv
                    * U1.T
                    * Q.T
            ...
            gcv_hes[j, k] = (
                -(2 * n_obs / (delta**3)) * delta_grad[k] * alpha_grad[j]
...
if __name__ == "__main__":
    ...
    smooth_pen = np.array([1.5, 0.51, 2])
    ...
    t0 = perf_counter()
    exog, endog, index_var = sm_handler.get_general_additive_model_endog_and_exog(
        var_list, z, weights=w
    )
    t1 = perf_counter()
    ...
    grad = gcv_grad_comp(
        rho0, X, Q, R, endog, sm_handler, var_list, return_par="alpha", gamma=gamma
    )
    func = lambda rho: gcv_comp(
        rho, X, Q, R, endog, sm_handler, var_list, return_par="alpha", gamma=gamma
    )
    ...
    grad = gcv_grad_comp(
        rho0, X, Q, R, endog, sm_handler, var_list, return_par="delta", gamma=gamma
    )
    func = lambda rho: gcv_comp(
        rho, X, Q, R, endog, sm_handler, var_list, return_par="delta", gamma=gamma
    )
    grad_app = approx_grad(rho0, grad.shape, func, 5 * 10**-3)
    if checkGrad(grad, grad_app, tol=10**-3):
    ...
    grad = gcv_grad_comp(
        rho0, X, Q, R, endog, sm_handler, var_list, return_par="gcv", gamma=gamma
    )
    func = lambda rho: gcv_comp(
        rho, X, Q, R, endog, sm_handler, var_list, return_par="gcv", gamma=gamma
    )
    grad_app = approx_grad(rho0, grad.shape, func, 5 * 10**-3)
    if checkGrad(grad, grad_app, tol=10**-3):
