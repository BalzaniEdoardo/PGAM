...
def computeKnots(
    ...
    for xx in X:
        ...
        if use_extern:
            ...
            delta = 0
        max_x = max_x + delta / 2.0
        min_x = min_x - delta / 2.0
        # any out of range?
...
def knots_x_condition(
    ...
    for cond_type in condition_type_list:
        ...
        if cond_type == "all":
            ...
            cond_value_list = np.unique(
                trial_type[cond_type][~np.isnan(trial_type[cond_type])]
            )
        ...
        X = [np.nan]
        for var in continuous_var:
            knots_cond[cond_type][var] = computeKnots(
                X,
                order,
                knots_num,
                1,
...
def knots_by_session(x, session, var, basis_info):
    ...
    if basis_info[session][var]["knots_type"] == "equi_freq":
        ...
        knots = equi_freq_knots(
            x, perc_0, perc_end, dkn_min, dkn_max, init_knots_num, order
        )
    elif basis_info[session][var]["knots_type"] == "equi_spaced":
        min_x = np.nanpercentile(x, perc_0)
        max_x = np.nanpercentile(x, perc_end)
        knots = equi_spaced_knots([x], min_x, max_x, init_knots_num, order)[
            0
        ]  # no 2d var
        ...
        knots = equi_spaced_knots(
            [x], min_x, max_x, init_knots_num, order, use_extern=False
        )[0]
    return knots
...
def equi_freq_knots(x, perc_0, perc_end, dkn_min, dkn_max, knots_num0, order):
    ...
    knots = np.hstack(
        ([final_knots[0]] * (order - 1), final_knots, [final_knots[-1]] * (order - 1))
    )
    return knots
...
def temporal_knots(knots_num, kernel_length, kernel_direction, order):
    ...
    repeats = order - 1
    kernel_length = (
        kernel_length + order + 1
    )  # + 8 # add few time points not to force the las time point to be zero
