...
def example_robust_WLS(w, z, X, S_list=[], lambda_list=[]):
    ...
    if S_list == []:
        ...
        try:
            ...
            E = E.T
        R, piv, piv_revert, Q = pivoted_QR(
            np.vstack((RR / np.linalg.norm(RR, "fro"), E / np.linalg.norm(E, "fro")))
        )
    ...
    if sele_row.sum() == 0:
        ...
        U, d, VT = linalg.svd(
            (Q1[sele_row, :].T * i_diag[sele_row]).T,
            full_matrices=False,
            overwrite_a=True,
        )
        ...
        if any(d > 1 / np.sqrt(2)):
            ...
            return robust_WLS(
                y, mu, family, X, S_list, lambda_list, fisher_scoring=True
            )
        else:
            # use full newton step
            P = np.dot(
                np.linalg.pinv(R[:, piv_revert_2]), VT.T / np.sqrt(1 - 2 * d**2)
            )
...
if __name__ == "__main__":
    ...
    beta, P, K, rank, a, b, c = example_robust_WLS(
        w, z, X, S_list=S_list, lambda_list=lams
    )
    ...
    for k in range(100):
        t0 = perf_counter()
        beta, P, K, rank, zz, ww, keep = robust_WLS(
            y,
            mu,
            family,
            X,
        ...
        t0 = perf_counter()
        beta1, z1, w1 = statsmodels_WLS(
            y, mu, family, X, S_list=S_list, lambda_list=lams, fisher_scoring=False
        )
        t1 = perf_counter()
