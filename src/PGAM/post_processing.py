...
def compute_tuning(spk, fit, exog, var, sm_handler, filter_trials, trial_idx, dt=0.006, bins=15):
    ...
    if fit.smooth_info[var]['is_temporal_kernel'] and fit.smooth_info[var]['is_event_input']:
        mu = np.dot(exog, fit.beta)
        sigma2 = np.einsum('ij,jk,ik->i', exog, fit.cov_beta, exog,
                           optimize=True)
        # convert to rate space
        lam_s = np.exp(mu + sigma2 * 0.5)
        # print('temp')
        ...
        x_axis, tuning, sc_based_tuning, counts = compute_tuning_temporal_fast(var, lam_s, sm_handler, fit, spk,
                                                                           filter_trials, trial_idx, bins, dt)
    else:
        # print('spat')
        mu = np.dot(exog[filter_trials], fit.beta)
            ...
            if var in reduced_fit.var_list:
                ...
                if reduced_fit.smooth_info[var]['is_temporal_kernel']:
                    ...
                    if (full_fit.smooth_info[var]['kernel_direction'] == 1) and \
                        ...
                        fminus = fminus - fX[-1]
                        fX = fX - fX[-1]
                    results['reduced_kernel_strength'][cc] = simps(fX ** 2, dx=full_fit.time_bin) / (
                                full_fit.time_bin * fX.shape[0])
                    results['reduced_signed_kernel_strength'][cc] = simps(fX, dx=full_fit.time_bin) / (
                                full_fit.time_bin * fX.shape[0])
