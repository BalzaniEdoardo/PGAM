...
def transform_Slam(S_all, rho):
    ...
    while True:
        ...
        # step 2
        try:
            select_alpha = Omega >= FLOAT_EPS ** (1 / 3.0) * Omega.max()
        except:
            print("Except raised")
            pass
        ...
        S_gamma_prime = np.einsum(
            "kih,k->ih", S_tild[index_gamma_prime], lam[index_gamma_prime]
        )
        ...
        T_alpha = np.block(
            [
                [np.eye(K), np.zeros((K, r)), np.zeros((K, totDim - K - r))],
                [np.zeros((Q, K)), Ur, np.zeros((Q, totDim - K - r))],
            ]
        )
        T_gamma = np.block([[np.eye(K), np.zeros((K, Q))], [np.zeros((Q, K)), U]])

        S_transformed[index_alpha] = np.einsum(
            "ji,rjk,kh->rih", T_alpha, S_transformed[index_alpha], T_alpha
        )
        S_transformed[index_gamma_prime] = np.einsum(
            "ji,rjk,kh->rih", T_gamma, S_transformed[index_gamma_prime], T_gamma
        )
