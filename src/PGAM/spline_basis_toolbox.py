...
class spline_basis_element(object):
    ...
    def __init__(self, basis_ID, knots, order, is_cyclic=False):
        if not is_cyclic:
            func = lambda knots, x, order: splineDesign(
                knots, x, ord=order, der=0, outer_ok=True
            )
        else:
        ...
        for k in range(len(int_knots) - 1):
            ...
            kkn1 = kn1 - (kn1 - kn0) / 10.0
            kkn0 = kn0 + (kn1 - kn0) / 10.0
        ...
        for i in range(num):
            ...
            for j in range(i, num):
                ...
                for knt in bi_2prime.keys():
                    ...
                    energy[i, j] = energy[i, j] + bij_2prime.integrate(
...
class tuning_function(object):
    def __init__(
        ...
        if subtract_integral_mean:
            ...
            self.beta[-1] = -self.integrate(knot_first, knot_last) / (
                knot_last - knot_first
            )
            num_basis = self.spline_basis.num_basis_elements
            self.spline_basis.basis[num_basis - 1] = spline_intercept_element(
                self.spline_basis.knots
            )
...
class between_knots_spline_basis_function(object):
    ...
    def __init__(self, coeff, domain):
        ...
        if domain.shape[1] != 2:
            raise ValueError(
                "Domain should be a vector of nx2 elements, representing the dijoint domain intervals"
            )
        if not self.is_disjoint(self.domain):
            raise ValueError("Domain intervals should be non overlapping")
        if domain.shape[0] > 1:
            raise ValueError(
                "spline basis is characterized by a single interval domain (a,b)"
            )
...
if __name__ == "__main__":
    ...
    sm_handler.add_smooth(
        ...
        knots_num=None,
        perc_out_range=None,
        is_cyclic=[basis_info[monkeyID][var]["is_cyclic"]],
        lam=None,
        penalty_type=penalty_type,
        der=der,
        trial_idx=np.ones(x.shape[0]),
        time_bin=0.006,
    ...
    exp_bspline = spline_basis(
        knots, order, is_cyclic=basis_info[monkeyID][var]["is_cyclic"]
    )
    path = (
        "/Volumes/WD Edo/firefly_analysis/LFP_band/cubic_gam_fit_with_coupling/gam_%s"
        % monkeyID
    )
    ...
    print(
        "should be zero", tuning.integrate(exp_bspline.knots[0], exp_bspline.knots[-1])
    )

    spline_basis_intercept = spline_basis(
        knots,
        order,
        is_cyclic=basis_info[monkeyID][var]["is_cyclic"],
    ...
    func = lambda x: (tuning_raw(x) - c / (knots[-1] - knots[0])) / np.sqrt(
        (knots[-1] - knots[0])
    )
    approx = simps(func(xx) ** 2, dx=xx[1] - xx[0])
    B_tild = spline_basis_intercept.integral_matrix(knots[0], knots[-1]) / (
        knots[-1] - knots[0]
    )

    sqrtB = smoothPen_sqrt(
        spline_basis_intercept.integral_matrix(knots[0], knots[-1])
    ) / np.sqrt(knots[-1] - knots[0])
    ...
    func2 = lambda x: (tuning_raw2(x) - c2 / (knots[-1] - knots[0])) / np.sqrt(
        (knots[-1] - knots[0])
    )
    ...
    exp_bspline_2 = spline_basis(
        knots_2, order, is_cyclic=basis_info[monkeyID][var]["is_cyclic"]
    )
    ...
    theo = (
        np.dot(np.dot(beta1, int_1), beta1)
    ...
    intercept_bspline = spline_basis(
        knots,
        order,
        is_cyclic=basis_info["m53s83"][var]["is_cyclic"],
        subtract_integral=True,
    )
    intercept_bspline_2 = spline_basis(
        knots_2,
        order,
        is_cyclic=basis_info["m53s93"][var]["is_cyclic"],
        subtract_integral=True,
    )
    ...
    approx = simps(
        ((tuning_raw2(xx) - c2) - (tuning_raw1(xx) - c1)) ** 2, dx=xx[1] - xx[0]
    )
    ...
    theo = (
        np.dot(np.dot(beta1_padded, int_1), beta1_padded)
