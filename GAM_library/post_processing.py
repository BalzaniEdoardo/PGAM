...
def compute_tuning(spk, fit, exog, var, sm_handler, filter_trials, trial_idx, dt=0.006, bins=15):
    ...
    if fit.smooth_info[var]['is_temporal_kernel'] and fit.smooth_info[var]['is_event_input']:
        # x_axis, tuning, sc_based_tuning, counts = compute_tuning_temporal(var, lam_s, sm_handler, fit, spk,
        ...
        # print('spat')
        x_axis, tuning, sc_based_tuning, counts = compute_tuning_spatial(var, lam_s, sm_handler, fit, spk,
                                                                         filter_trials, bins, dt)
...
def mutual_info_est(spk_counts, exog, fit, var, sm_handler, filter_trials, trial_idx, dt=0.006, bins=15,
    ...
    # print('here')
    temp_bins, tuning, sc_based_tuning, count_bins = compute_tuning(spk_counts, fit, exog, var, sm_handler,
                                                                    filter_trials, trial_idx, dt=0.006, bins=bins)
    tuning = tuning * dt
...
def alignRateForMI(y, lam_s, var, sm_handler, smooth_info, time_bin, filter_trials, trial_idx):
    ...
    for tr in np.unique(trial_idx):
        ...
        for ii in np.where(rwd_tr == 1)[0]:
            ...
            tmpy = y_tr[i0:i1]
            iidx = np.array(
                np.round(nbin // 2 + (-d0 + np.arange(0, d1 + d0)) * time_bin / (temp_bins[1] - temp_bins[0])),
                dtype=int)
...
def compute_tuning_temporal(var, lam_s, sm_handler, fit, spk, filter_trials, trial_idx, bins, dt):
    ...
    while flag:
        ...
        if not flag:
            break
        tuning, sc_based_tuning, tot_s_vec = compute_average(spk[filter_trials], lam_s, events_analyze, temp_bins,
                                                             sc_based_tuning, tuning, tot_s_vec, half_size, timept,
...
def postprocess_results(neuron_id, counts, full_fit, reduced_fit, train_bool,
    ...
    for cc in range(len(full_fit.var_list)):
        ...
        try:
            mi_full, tun_full = mutual_info_est(counts, exog_full, full_fit, var, sm_handler, train_bool, trial_idx,
        ...
        (results['x_kernel'][cc], results['y_kernel'][cc],
         ...
         results['kernel_strength'][cc], results['signed_kernel_strength'][cc]) = prediction_and_kernel_str(full_fit,
                                                                                                            var,
                                                                                                            var_zscore_par)
        ...
        if not (reduced_fit is None):
            ...
            try:
                mi_red, tun_red = mutual_info_est(counts, exog_reduced, reduced_fit, var, sm_handler, train_bool,
                                                  trial_idx,
                                                  dt=full_fit.time_bin, bins=bins)
                if ~np.isnan(var_zscore_par[var]['loc']):
                    xx_scaled = []
...
def compute_event_trig_counts(ev_receiver, pred_rate_receiver, idx_spk, rate_DT,
                              counts_DT, tp_DT, tot_tp, delta_step, skip_t0):
    """
...
def compute_tuning_temporal_fast(var, lam_s, sm_handler, fit, spk, filter_trials, trial_idx, bins, dt):
    ...
    skip_t0 = (var == 'spike_hist') #or var.startswith("neu_")
...
def align_rates_directional(events, filter_len, rate, trial_idx):
    ...
    trial_event = trial_idx[event_idx]

    unq_trial = np.unique(trial_idx)
    trial_end_dict = {}
    for tr in unq_trial:
        idx_trial = np.where(trial_idx == tr)[0]
        trial_end_dict[tr] = idx_trial[-1]

    rate_mat = np.zeros((event_idx.shape[0], filter_len))*np.nan

    for n_event, event_start in enumerate(event_idx):
        trial = trial_event[n_event]
        i0 = event_start + 1
        if i0 >= len(rate):
            break
        i1 = min(event_start + 1 + filter_len, trial_end_dict[trial])
        if i1 <= i0:
            continue
        rate_mat[n_event, :(i1 - i0)] = rate[i0:i1]
    return rate_mat


def align_rates_centered(events, filter_len, rate, trial_idx):
    event_idx = np.where(events > 0)[0]
    trial_event = trial_idx[event_idx]
    ...
    for tr in unq_trial:
        idx_trial = np.where(trial_idx == tr)[0]
        trial_start_dict[tr] = idx_trial[0]
        trial_end_dict[tr] = idx_trial[-1]

    half_len = filter_len // 2

    rate_mat = np.zeros((event_idx.shape[0], 2*half_len+1)) * np.nan

    for n_event, event_center in enumerate(event_idx):
        trial = trial_event[n_event]

        i0 = max(event_center - half_len, trial_start_dict[trial])
        i1 = min(event_center + half_len + 1, trial_end_dict[trial])

        start_col = max(0, half_len - (event_center - i0))
        end_col = start_col + (i1 - i0)

        rate_mat[n_event, start_col:end_col] = rate[i0:i1]

    return rate_mat

def average_in_bins(data, bin_size):
    """
    Compute the average over data in a certain number of bins using nanmean.

    Parameters:
    ----------
    - data: numpy array (e.g., rate_mat output from previous functions)
    - bin_size: size of each bin

    Returns:
    --------
    - binned_avg: averaged data with shape (data.shape[0], num_bins)
    """

    # Calculate number of bins based on bin_size
    num_bins = data.shape[1] // bin_size + (data.shape[1] % bin_size != 0)

    binned_avg = np.zeros((data.shape[0], num_bins)) * np.nan

    for i in range(num_bins):
        start_idx = i * bin_size
        end_idx = start_idx + bin_size
        binned_avg[:, i] = np.nanmean(data[:, start_idx:end_idx], axis=1)

    return binned_avg


def compute_tuning_centered(events, rate, filter_len, bins, trial_idx, dt):
    rate_align = align_rates_centered(
        events,
        filter_len, rate,
        trial_idx
    )
    ...
    for idx, tindex in enumerate(unq_times):
        counts[idx] = (~np.isnan(rate_align[:, time_idx == tindex].flatten())).sum()
        tuning[idx] = np.nanmean(rate_align[:, time_idx == tindex]) / dt
    time_sec = unq_times * filter_len / bins * 0.006
    return time_sec, tuning, counts
