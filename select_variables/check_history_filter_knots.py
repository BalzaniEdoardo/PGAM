...
for neuron in neuron_list:
    ...
    full_fewKnots, reduced_fewKnots = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
    ...
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_fewKnots.var_list)
    spk = yt[keep_test,neuron-1]
    p_r2_fewKnots = pseudo_r2_compute(spk,family, modelX_test,full_fewKnots.beta)
    ...
    for var in np.hstack((var_names, ['lfp_beta','lfp_alpha','lfp_theta','spike_hist'])):
        ...
        knots, x_trans, include_var, is_cyclic, order,\
            kernel_len,kernel_direction,is_temporal_kernel,penalty_type,der =\
                knots_cerate(x,var,session,hand_vel_temp=hand_vel_temp,hist_filt_dur='long_many',
                exclude_eye_position=['m44s213','m53s133','m53s134','m53s105','m53s182'])
    ...
    full_manyKnots, reduced_manyKnots = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
                                                        conv_criteria='gcv',
                                                        max_iter=10000, gcv_sel_tol=10 ** -13, random_init=False,
                                                        use_dgcv=True, initial_smooths_guess=False,
                                                        fit_initial_beta=True, pseudoR2_per_variable=True,
    ...
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_manyKnots.var_list)
    spk = yt[keep_test,neuron-1]
    p_r2_manyKnots = pseudo_r2_compute(spk,family, modelX_test,full_manyKnots.beta)
    
    with open('compare_hist_filt_manyKnots_%s_c%d.dill'%(session,neuron),'wb') as fh:
       
            data_dict = {
              'full_fewKnots':full_fewKnots,
              'reduced_fewKnots':reduced_fewKnots,
              'full_manyKnots':full_manyKnots,
              'reduced_manyKnots':reduced_manyKnots,
              'p_r2_manyKnots':p_r2_manyKnots,
              'p_r2_fewKnots':p_r2_fewKnots,
