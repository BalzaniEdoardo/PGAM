...
try:
    ...
    folder_name = ''
    sv_folder_base = ''
    fhName = os.path.join('/Volumes/WD_Edo/firefly_analysis/LFP_band/concatenation_with_accel/','m53s105.npz')
...
for var in np.hstack((var_names, ['lfp_beta','lfp_alpha','lfp_theta','spike_hist'])):
    ...
    knots, x_trans, include_var, is_cyclic, order,\
        kernel_len,kernel_direction,is_temporal_kernel,penalty_type,der =\
            knots_cerate(x,var,session,hand_vel_temp=hand_vel_temp,
...
full_spatial, reduced_spatial = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
...
modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_spatial.var_list)
spk = yt[keep_test,neuron-1]
p_r2_spatial = pseudo_r2_compute(spk,family, modelX_test,full_spatial.beta)
...
full_temporal, reduced_temporal = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
                                                    conv_criteria='gcv',
                                                    max_iter=10000, gcv_sel_tol=10 ** -13, random_init=False,
                                                    use_dgcv=True, initial_smooths_guess=False,
                                                    fit_initial_beta=True, pseudoR2_per_variable=True,
                                                    trial_num_vec=trial_idx_train, k_fold=use_k_fold, fold_num=num_folds,
                                                    reducedAdaptive=reducedAdaptive,compute_MI=False)

modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_temporal.var_list)
spk = yt[keep_test,neuron-1]
p_r2_temporal = pseudo_r2_compute(spk,family, modelX_test,full_temporal.beta)

with open('compare_hand_vel_%s_c%d.dill'%(session,neuron),'wb') as fh:
   
        data_dict = {
          'full_spatial':full_spatial,
          'reduced_spatial':reduced_spatial,
          'full_temporal':full_temporal,
          'reduced_temporal':reduced_temporal,
          'p_r2_temporal':p_r2_temporal,
          'p_r2_spatial':p_r2_spatial,
