...
for neuron in neuron_list:
    ...
    full_long, reduced_long = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
    ...
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_long.var_list)
    spk = yt[keep_test,neuron-1]
    p_r2_long = pseudo_r2_compute(spk,family, modelX_test,full_long.beta)
    ...
    full_short, reduced_short = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
                                                        conv_criteria='gcv',
                                                        max_iter=10000, gcv_sel_tol=10 ** -13, random_init=False,
                                                        use_dgcv=True, initial_smooths_guess=False,
                                                        fit_initial_beta=True, pseudoR2_per_variable=True,
    ...
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_short.var_list)
    spk = yt[keep_test,neuron-1]
    p_r2_short = pseudo_r2_compute(spk,family, modelX_test,full_short.beta)
    
    with open('compare_hist_filt_%s_c%d.dill'%(session,neuron),'wb') as fh:
