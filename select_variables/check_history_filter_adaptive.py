...
for neuron in neuron_list:
    ...
    full_derivative, reduced_derivative = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
    ...
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_derivative.var_list)
    spk = yt[keep_test,neuron-1]
    p_r2_derivative = pseudo_r2_compute(spk,family, modelX_test,full_derivative.beta)
    ...
    full_adaptive, reduced_adaptive = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
                                                        conv_criteria='gcv',
                                                        max_iter=10000, gcv_sel_tol=10 ** -13, random_init=False,
                                                        use_dgcv=True, initial_smooths_guess=False,
                                                        fit_initial_beta=True, pseudoR2_per_variable=True,
    ...
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_adaptive.var_list)
    spk = yt[keep_test,neuron-1]
    p_r2_adaptive = pseudo_r2_compute(spk,family, modelX_test,full_adaptive.beta)
    
    with open('compare_hist_filt_adaptive_%s_c%d.dill'%(session,neuron),'wb') as fh:
       
            data_dict = {
              'full_derivative':full_derivative,
              'reduced_derivative':reduced_derivative,
              'full_adaptive':full_adaptive,
              'reduced_adaptive':reduced_adaptive,
              'p_r2_adaptive':p_r2_adaptive,
              'p_r2_derivative':p_r2_derivative,
