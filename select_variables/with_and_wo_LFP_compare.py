...
try:  # IF CLUSTER JOB IS RUNNING
    ...
    list_condition = np.load(os.path.join('/scratch/jpn5/fit_lfp_vs_coupling','condition_list_%s.npy' % session))
...
for neuron in neuron_list:
    ...
    for ba in other_ba:
        ...
        for freq in ['theta', 'alpha', 'beta']:
            ...
            knots, x_trans, include_var, is_cyclic, order, \
            kernel_len, kernel_direction, is_temporal_kernel, penalty_type, der = \
                knots_cerate(x, 'lfp_%s' % freq, session, hand_vel_temp=hand_vel_temp, hist_filt_dur='long',
                             exclude_eye_position=['m44s213', 'm53s133', 'm53s134', 'm53s105', 'm53s182'],
    ...
    for var in np.hstack((var_names, ['lfp_beta', 'lfp_alpha', 'lfp_theta', 'spike_hist'])):
        ...
        if include_var:
            ...
            sm_handler_test.add_smooth(var, [x_test], ord=order, knots=[knots],
                                       ...
                                       trial_idx=trial_idx_test, time_bin=time_bin,
                                       is_temporal_kernel=is_temporal_kernel,
                                       kernel_length=kernel_len, kernel_direction=kernel_direction, ord_AD=3,
                                       ad_knots=4)
    ...
    full_spike_hist, reduced_spike_hist = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001,
                                                                     ...
                                                                     use_dgcv=True, initial_smooths_guess=False,
                                                                     fit_initial_beta=True, pseudoR2_per_variable=True,
                                                                     trial_num_vec=trial_idx_train, k_fold=use_k_fold,
                                                                     fold_num=num_folds,
    ...
    with open('compare_w_wo_LFP_%s_c%d.dill'%(session,neuron),'wb') as fh:
       
            data_dict = {
              'full_input':full_input,
              'reduced_input':reduced_input,
              'full_within_coupling':full_spike_hist,
              'reduced_coupling':reduced_spike_hist,
              'full_all_LFP':full_coupling,
              'reduced_all_LFP':reduced_coupling,
              'p_r2_all_LFP':p_r2_coupling,
              'p_r2_coupling':p_r2_spike_hist,
