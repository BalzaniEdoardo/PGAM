...
try:
    folder_name = '/scratch/jpn5/select_hand_vel' #user_paths.get_path('data_hpc')
...
for neuron in neuron_list:
    ...
    fh = open('/Volumes/WD_Edo/firefly_analysis/LFP_band/GAM_history_filter_ML/compare_hist_filt_ML_m53s95_c%d.dill'%(neuron),'rb') 
    res = dill.load(fh)
    fh.close()
    full = res['full_PL']
    
    XX,idx_dict = sm_handler.get_exog_mat(sm_handler.smooths_var)
    mu = np.exp(np.dot(XX, full.beta))
    family.loglike(yt[keep,neuron-1],mu)
    family.loglike(yt[keep,neuron-1],[np.mean(yt[keep,neuron-1])]*keep.shape[0])/(yt[:,neuron-1].sum() * np.log2(np.exp(1)))
    ...
    full_test, reduced_test = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
                                                        conv_criteria='gcv',compute_MI=False,
                                                        max_iter=0, gcv_sel_tol=10 ** -13, random_init=False,
                                                        ...
                                                        reducedAdaptive=False,perform_PQL=False)
    
    mu = np.exp(np.dot(XX, full.beta))
    ll_pql = family.loglike(yt[keep,neuron-1],mu)/(yt[:,neuron-1].sum() * np.log2(np.exp(1)))
    mu_test = np.exp(np.dot(XX, full_test.beta))
    ll_test = family.loglike(yt[keep,neuron-1],mu_test)/(yt[:,neuron-1].sum() * np.log2(np.exp(1)))
    
    modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_test.var_list)
    modelX, idx_dict = sm_handler.get_exog_mat(full_test.var_list)
    
    
    spk = yt[keep,neuron-1]
    p_r2_nocv_test = pseudo_r2_compute(spk,family, modelX,full_test.beta)
    p_r2_nocv_pql = pseudo_r2_compute(spk,family, modelX, full.beta)


    spk = yt[keep_test,neuron-1]
    p_r2_test = pseudo_r2_compute(spk,family, modelX_test, full_test.beta)
    p_r2_pql = res['p_r2_PL']
    ...
    #         sm_handler.add_smooth(var, [x_trans], ord=order, knots=[knots], 
    #                               knots_num=None, perc_out_range=None,
    #                       is_cyclic=[is_cyclic], lam=0.000001, 
    #                       penalty_type=penalty_type,
    #                       der=der,
    #                       trial_idx=trial_idx_train, time_bin=time_bin, 
    #                       is_temporal_kernel=is_temporal_kernel,
    ...
    #         sm_handler_test.add_smooth(var, [x_test], ord=order, knots=[knots], 
    #                               knots_num=None, perc_out_range=None,
    #                       is_cyclic=[is_cyclic], lam=.000001, 
    #                       penalty_type=penalty_type,
    #                       der=der,
    #                       trial_idx=trial_idx_test, time_bin=time_bin, 
    #                       is_temporal_kernel=is_temporal_kernel,
    ...
    # full_ML, reduced_ML = gam_model.fit_full_and_reduced(sm_handler.smooths_var, th_pval=0.001, method='L-BFGS-B', tol=1e-8,
    #                                                     conv_criteria='gcv',
    #                                                     max_iter=0, gcv_sel_tol=10 ** -13, random_init=False,
    #                                                     use_dgcv=True, initial_smooths_guess=False,
    #                                                     fit_initial_beta=True, pseudoR2_per_variable=True,
    #                                                     trial_num_vec=trial_idx_train, k_fold=use_k_fold, fold_num=num_folds,
    #                                                     reducedAdaptive=False,compute_MI=False,perform_PQL=False)
    
    # modelX_test, idx_dict_test = sm_handler_test.get_exog_mat(full_ML.var_list)
    # spk = yt[keep_test,neuron-1]
    # p_r2_ML = pseudo_r2_compute(spk,family, modelX_test,full_ML.beta)
    
    # with open('compare_hist_filt_ML_%s_c%d.dill'%(session,neuron),'wb') as fh:
       
    #         data_dict = {
    #           'full_PL':full_PL,
    #           'reduced_PL':reduced_PL,
    #           'full_ML':full_ML,
