...
h = h / np.sum(h)
var = 't_reward'
for var in ['t_reward','t_stop','t_move','t_flyOFF']:
    ...
    for session in sess_list[select]:
        ...
        num_vip = ( unit_info['brain_area']  == 'VIP').sum()
        ...
        for kk in range(resampling):
            ...
            sm_spk_vip = sm_spikes[:, unit_info['brain_area'] == 'VIP']
            if num_ppc > num_vip:
            ...
            if kk == 0:
                ...
                if refit_ppc:
                    train_spk = sm_spk_vip[bool_train]
                    test_spk = sm_spk_vip[bool_test]
        ...
        print(session, 'PPC:',num_ppc, 'VIP:',num_vip)
        print(score_session[session])
               # if kk == 0:
               #     model = lasso(alpha=alpha)
               #     res = model.fit(sm_spk_vip[bool_train], eye_pos[bool_train])
               #     scr = res.score(sm_spk_vip[bool_test], eye_pos[bool_test])
               #     score_session[session]['PFC'] = scr
               #     score_session[session]['PPC'] = []
    
    
    np.save('VIP_decoding_linSVM_%s.npy'%var,score_session)