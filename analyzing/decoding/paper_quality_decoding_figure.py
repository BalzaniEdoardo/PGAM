...
for var in ['rad_acc','rad_vel','t_stop','t_flyOFF','eye_hori']:#,'eye_vert']:
...
fig = plt.figure(figsize=[8.9 , 4.83])
...
g= sns.violinplot(data=dat,hue='area', x="variable", y="fractional increase", split=False,
            palette=palette,legend_out=True,ax=ax1)

ax1.spines['top'].set_visible(False)
ax1.spines['right'].set_visible(False)

xlim = plt.xlim()
ax1.plot(xlim,[0,0],'--r',lw=2)
plt.suptitle('fractional increase in decoding performace')
plt.tight_layout(rect=[0, 0.05, 0.90, 0.95])
plt.savefig('decoding_pecent_increment.png')
