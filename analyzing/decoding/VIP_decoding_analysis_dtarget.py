...
def hist_equating_selection(ppc_rates,vip_rates):
    
    min_rate = np.min(np.hstack((ppc_rates,vip_rates)))
    max_rate = np.max(np.hstack((ppc_rates,vip_rates)))
    ...
    for k in range(range_bin.shape[0]-1):
        ...
        idx_vip = np.where((vip_rates >= range_bin[k]) * (vip_rates < range_bin[k+1]))[0]
        
        num_sele = min(idx_vip.shape[0],idx_ppc.shape[0])
        
        if num_sele==0:
            continue
        
        sele_ppc = np.hstack((sele_ppc,np.random.choice(idx_ppc,size=num_sele,replace=False)))
        sele_vip = np.hstack((sele_vip,np.random.choice(idx_vip,size=num_sele,replace=False)))

    sele_vip = np.array(sele_vip,dtype=int)
    sele_ppc = np.array(sele_ppc,dtype=int)
    return sele_ppc,sele_vip
...
var = 'rad_path'
for var in ['rad_path','eye_vert','eye_hori','rad_acc','rad_vel']:
    ...
    thresh = num_unit_x_area  >= 19
    select = thresh[:,1] & thresh[:,3] 
    ...
    for session in sess_list[select]:
        ...
        firing_rates[session]['VIP'] = (spikes[:,brain_area=='VIP']).mean(axis=0)/0.006
        ...
        num_vip = (brain_area == 'VIP').sum()
        
       
        
        alpha = 0.001
        score_session[session] = {}
        hist_matched_firing_rates[session] = {'PPC':[],'VIP':[]}
        
        for kk in range(resampling):
            print(kk,resampling)
            
            idx_ppc,idx_vip = hist_equating_selection(firing_rates[session]['PPC'], firing_rates[session]['VIP'])
        
            sm_spk_ppc = sm_spikes[:, brain_area == 'PPC']
            sm_spk_vip = sm_spikes[:, brain_area == 'VIP']
            
            if num_ppc > num_vip:
               # sub_sel = np.random.choice(np.arange(num_ppc), size=num_vip, replace=False)
               # sm_spk_ppc = sm_spk_ppc[:, sub_sel]
               sm_spk_ppc = sm_spk_ppc[:, idx_ppc]
               sm_spk_vip = sm_spk_vip[:, idx_vip]
               
               hist_matched_firing_rates[session]['PPC'] += [sm_spk_ppc.mean(axis=0)]
               hist_matched_firing_rates[session]['VIP'] += [sm_spk_vip.mean(axis=0)]
    
               
               if kk == 0:
                   print('PPC num', idx_ppc.shape[0],'VIP num', idx_vip.shape[0])
                   model = lasso(alpha=alpha)
                   res = model.fit(sm_spk_vip[bool_train], rad_acc[bool_train])
                   scr = res.score(sm_spk_vip[bool_test], rad_acc[bool_test])
                   score_session[session]['VIP'] = scr
            ...
            elif num_ppc < num_vip:
               # sub_sel = np.random.choice(np.arange(num_vip),size=num_ppc,replace=False)
               # sm_spk_vip = sm_spk_vip[:, sub_sel]
               sm_spk_ppc = sm_spk_ppc[:, idx_ppc]
               sm_spk_vip = sm_spk_vip[:, idx_vip]
               ...
               if kk == 0:
                    ...
                    score_session[session]['VIP'] = []
                    
               model = lasso(alpha=alpha)
               res = model.fit(sm_spk_vip[bool_train], rad_acc[bool_train])
               scr = res.score(sm_spk_vip[bool_test], rad_acc[bool_test])
               score_session[session]['VIP'] = np.hstack((score_session[session]['VIP'], [scr]))
    ...
    np.save('VIP_hist_matched_decoding_%s_with_subsamp.npy'%var, score_session)
    np.savez('VIP_firing_rate_pop.npz',firing_rates=firing_rates,hist_matched_firing_rates=hist_matched_firing_rates) 
