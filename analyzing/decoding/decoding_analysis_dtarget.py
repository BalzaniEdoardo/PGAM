...
for session in sess_list[select]:
    ...
    hist_matched_firing_rates[session] = {'PPC':[],'PFC':[]}
    ...
    for kk in range(resampling):
        ...
        if num_ppc > num_pfc:
           ...
           hist_matched_firing_rates[session]['PFC'] += [sm_spk_pfc.mean(axis=0)]

           
           if kk == 0:
               print('PPC num', idx_ppc.shape[0],'PFC num', idx_pfc.shape[0])
...
np.save('hist_matched_decoding_%s_with_subsamp.npy'%var, score_session)
np.savez('firing_rate_pop.npz',firing_rates=firing_rates,hist_matched_firing_rates=hist_matched_firing_rates) 
