...
from spline_basis_toolbox import spline_basis, tuning_function
...
cnt_done = 0
session_list = []
for sessionfld in lst_done:
    if not sessionfld.startswith('gam_'):
        continue
    session = sessionfld.split('gam_')[1]
...
for session in session_list:
    ...
    first = True
    count_fit = 0
    info_dict[session] = np.zeros(len_fits,dtype=dtype_info)
    progress = ProgressBar(len_fits)

    print(session, 'computing beta and integral x neuron...')
    remove_bool = np.ones(len_fits,dtype=bool)
    for fname in fits:
        ...
        if first:
            ...
            for var in np.hstack((spatial_list, extra_var)):
                ...
                if not var in beta_dict.keys():
                    beta_dict[var] = {session: np.zeros((len_fits, beta_len))}
                    int_tuning_dict[var] = {session: np.zeros(len_fits)}
                    knots_dict[var] = {session: np.zeros(len_fits,dtype=object)}
                    tuning_func_dict[var] = {session :np.zeros(len_fits, dtype=object)}
                else:
                    beta_dict[var][session] = np.zeros((len_fits, beta_len))
                    int_tuning_dict[var][session] = np.zeros(len_fits)
                    knots_dict[var][session] = np.zeros(len_fits,dtype=object)
        ...
        for var in np.hstack((spatial_list,extra_var)):
            try:
                ...
                if gam_res is None:
                    ...
                    order = gam_res.smooth_info[var]['ord'] #basis_info[session][var]['order']
                    is_cyclic = gam_res.smooth_info[var]['is_cyclic'][0]
                    exp_bspline = spline_basis(knots, order, is_cyclic=is_cyclic)
                    tuning = tuning_function(exp_bspline, np.hstack((beta,[0])), 
...
for i in range(session_list.shape[0]):
    ...
    for j in range(i,session_list.shape[0]):
        ...
        for var in np.hstack((spatial_list, extra_var)):
            ...
            with open(os.path.join(base_fld % session_1, fit1), 'rb') as dill_fh:
                gam_dict = dill.load(dill_fh)
            full = gam_dict['full']
            try:
                order = full.smooth_info[var]['ord']
                knots = full.smooth_info[var]['knots'][0]
                is_cyclic = full.smooth_info[var]['is_cyclic'][0]
            ...
            with open(os.path.join(base_fld % session_2, fit2), 'rb') as dill_fh:
                gam_dict = dill.load(dill_fh)
            full = gam_dict['full']
            try:
                knots = full.smooth_info[var]['knots'][0]
...
progress.done()
#'tuning_func_dict':tuning_func_dict
res = {'int_matrix':int_matrix_dict,'range_dict':range_dict,'int_tuning':int_tuning_dict,
         'beta_dict':beta_dict,'info_dict':info_dict,'knots_dict':knots_dict}
with open('preprocesed_session_density_all.dill','wb') as dill_fh:
