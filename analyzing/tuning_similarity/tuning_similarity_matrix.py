...
if check_tuning_sim:
    ...
    row, col = np.indices(pairwise_dist[(session_i, session_j)][:, :, idx].shape)
    ...
    idx0 = np.where(pairwise_dist[(session_i, session_j)][sort_i, sort_j, idx] > 0.01)[0][0]
    ...
    plt.suptitle('L2 distance: sorted by similarity - %s' % var_list[idx])
    for j in range(idx0+0, idx0 + 25):
        try:
            plt.subplot(5, 5, cc)
            neu_1 = neu_vector_i[row[idx_sort[j]]]
            neu_2 = neu_vector_j[col[idx_sort[j]]]
            plt.title('%d - %d: %.4f' % (neu_1, neu_2, pairwise_dist[(session_i, session_j)][sort_i[j], sort_j[j], idx]), fontsize=10)
