...
if os.path.exists('/Users/edoardo/Work/Code/GAM_code/GAM_library'):
    ...
    fit_res_fld = '/Volumes/WD_Edo/firefly_analysis/LFP_band/GAM_fit_with_acc'
    ...
    fld_file = '/scratch/jpn5/mutual_info/done/'
    fit_res_fld = '/scratch/jpn5/mutual_info/'
    fld_npz = '/scratch/jpn5/dataset_firefly'
    JOB = int(sys.argv[1])
...
#     res = dill.load(open(os.path.join(fit_res_fld,'gam_%s/fit_results_%s_c%d_all_1.0000.dill'%(session,session,neuron)),'rb'))
#
#     reduced = res['reduced']
...
dtype_dict={'names':('session','neuron','variable','significant','fit_type','pseudo-r2'),
            'formats':('U20',int,'U20',bool,'U20',float)}
...
for session,neuron in list_use:
    ...
    res = dill.load(open(os.path.join(fit_res_fld,
        'gam_%s/fit_results_%s_c%d_all_1.0000.dill' % (
        session, session, neuron)), 'rb'))

    reduced = res['reduced']

    fhName = os.path.join(fld_npz,'%s.npz'%session)
    ...
    for var in full_coupling.var_list:
        ...
        tmp['pseudo-r2'][cc_tmp] = full_coupling.pseudo_r2
        cc_tmp +=1
