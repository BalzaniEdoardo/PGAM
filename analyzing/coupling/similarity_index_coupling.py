...
sel = np.ones(info.shape,dtype=bool)
for sess in np.unique(info['session']):
    if 'm51' in sess:
        sel[info['session']==sess] = False
...
cond_HD_idx = 4
cond_LD_idx = 3
print('HD cond', cond_list[cond_HD_idx])
print('LD cond', cond_list[cond_LD_idx])
...
for ba in ['MST','PFC','PPC']:
    ...
    dict_regr_rsq[ba] = np.zeros(inf_ba.shape[0], dtype=float)*np.nan
    dict_session[ba] = np.zeros(inf_ba.shape[0], dtype='U30')
    for cc in range(inf_ba.shape[0]):
        if (not inf_ba['is sign density 0.0001'][cc]) or ( not inf_ba['is sign density 0.0050'][cc]):
            continue
        ...
        yy = tun_ba[cc, cond_LD_idx,:-2] * lreg.slope + lreg.intercept


        RSQ = 1 - np.sum((yy - tun_ba[cc, cond_HD_idx,:-2] ) ** 2) / np.sum(
            (tun_ba[cc, cond_HD_idx,:-2] - np.mean(tun_ba[cc, cond_HD_idx,:-2])) ** 2)

        dict_regr_rsq[ba][cc] = RSQ
        dict_session[ba][cc] = inf_ba[cc]['session']
...
for ba in ['MST','PFC','PPC']:
    ...
    dict_session[ba] = dict_session[ba][(dict_session[ba] != '')]
...
for ba in ['MST','PFC','PPC']:
    ...
    ax1.plot(x_corr, cdf_corr(x_corr),color=ba_color[ba])
    ax2.plot(x_slope, cdf_slope(x_slope),color=ba_color[ba])
    ax3.plot(x_intercept, cdf_intercept(x_intercept),color=ba_color[ba])
...
plt.title('R-squared')
for ba in ['MST','PFC','PPC']:
    cdf_rsq = ECDF(dict_regr_rsq[ba][~np.isnan(dict_regr_rsq[ba])])

    xmin_rsq = np.nanpercentile(dict_regr_rsq[ba], 0.)
    xmax_rsq = np.nanpercentile(dict_regr_rsq[ba], 100)
    x_rsq = np.linspace(xmin_rsq, xmax_rsq, 1000)
    plt.plot(x_rsq,cdf_rsq(x_rsq),color=ba_color[ba])
    
savemat('correlation_dict.mat', mdict={'corr_dict':dict_corr,'session_dict':dict_session})