...
unq_session = np.unique(info['session'])
for session in unq_session:
    ...
    for ba in ['MST','PFC','PPC']:
        sel = (info['sender brain area'] == ba) & (info['is significant']) & (info['pseudo-r2']>0.001) & (info['monkey'] == 'Schro') & (info['session']==session)
        if sel.sum() < 5 :
            do_break = True
            break
        tun_ba = tunings[sel]
    ...
    plt.suptitle('coupling with %s'%session)
    ax1 = plt.subplot(131)
    ax2 = plt.subplot(132)
    ax3 = plt.subplot(133)
    ...
    plt.savefig('/Users/edoardo/Work/Code/GAM_code/analyzing/coupling/fig_session_all/%s_cdf_coupling.png'%session)
    plt.close('all')
    # plt.figure()
    # for ba in ['MST','PFC','PPC']:
