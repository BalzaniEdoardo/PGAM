...
monkey='Quigley'
dat = np.load('/Users/edoardo/Work/Code/GAM_code/analyzing/coupling/coupling_x_cond/coupling_x_cond.npy',
              allow_pickle=True)
...
cc = 0
for area_sender in ['MST','PPC','VIP']:
    dat_send = dat_cond[dat_cond['sender brain area'] == area_sender]
    for area_receiver in ['MST','PPC','VIP']:
        ...
        ((~dat_filt[cond_1_lab]) & (~dat_filt[cond_1_lab])).sum()
...
writer = pd.ExcelWriter('counts_coupling.xlsx')
