...
if os.path.exists('coupling_info.npy'):
    info_coupling = np.load('coupling_info.npy')
    done_id = []
    for k in range(info_coupling.shape[0]):
        done_id += ['%s_%d_%s_%s'%(info_coupling['session'][k],
                                   info_coupling['receiver unit id'][k],
                                   info_coupling['manipulation type'][k],
                                   info_coupling['manipulation value'][k]
                                   )]
...
for (root,dirs,files) in os.walk(path_to_gam):
    ...
    np.save('coupling_info.npy',info_coupling)
