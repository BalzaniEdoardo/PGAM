...
@author: edoardo
"""
print ('THIS CODE IS WRONG, NEED TO CREATE THE MODEL MATIRX TO GET CORRECT\
       PREDICITON. ')
testMode=False
...
sel_trial = trial_idx_test < np.unique(trial_idx_test)[22]
spk = spikes[keep_test]
flt = pop_spike_convolve(spk[sel_trial],trial_idx_test[sel_trial],h)
...
neuronvec_id = np.zeros(spk.shape[1],dtype=int)
...
# # 30 sec
sm_predict = pop_spike_convolve(predict,trial_idx_test[sel_trial],h)
...
smModel_plot[:,brain_area_coupl_flt=='MST'] = np.clip(smModel_plot[:,brain_area_coupl_flt=='MST'],0,49.9)
smModel_plot[:,brain_area_coupl_flt=='PPC'] =  np.clip(50 + smModel_plot[:,brain_area_coupl_flt=='PPC'],50,99.9)
smModel_plot[:,brain_area_coupl_flt=='PFC'] =  np.clip(100 + smModel_plot[:,brain_area_coupl_flt=='PFC'],100,150)


smAll_plot[:,brain_area_coupl_flt=='MST'] = np.clip(smAll_plot[:,brain_area_coupl_flt=='MST'],0,49.9)
smAll_plot[:,brain_area_coupl_flt=='PPC'] =  np.clip(50+ smAll_plot[:,brain_area_coupl_flt=='PPC'],50,99.9)
smAll_plot[:,brain_area_coupl_flt=='PFC'] =  np.clip(100 + smAll_plot[:,brain_area_coupl_flt=='PFC'],100,150)
...
plt.tight_layout()  
# plt.savefig('test_%s_60sec_raw_vs_model.tiff'%session)
