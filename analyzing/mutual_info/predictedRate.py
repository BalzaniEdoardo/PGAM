...
for within_area in ['PPC','PFC','MST','VIP']:
    ...
    for neuron in  keep_unit[brain_area==within_area]:
        ...
        rate,pr2 = compute_predicted_rate(spikes_neu, sm_handler, modelX, trial_type,
                            trial_idx_train, index_dict, neuron,
                            folder_fh, session, cond_dict, monkey_dict, within_area)
        brain_area_res[ccn] = within_area
        neurin_ids[ccn] = keep_unit[idx_neu]
        rate_matrix[:,idx_neu] = rate
        spikes_raw[:,idx_neu] = spikes_neu
        pr2_matrix[idx_neu] = pr2
...
smAll = pop_spike_convolve(spikes_raw,trial_idx_train,h)
sm_model = pop_spike_convolve(rate_matrix,trial_idx_train,h)
# sm_model = rate_matrix
...
filt_trials = trial_idx_train<np.unique(trial_idx_train)[22]
smAll_plot = smAll[filt_trials]/0.006
smModel_plot = sm_model[filt_trials]/0.006

smAll_plot = smAll_plot[:,pr2_matrix>0.07]
smModel_plot = smModel_plot[:,pr2_matrix>0.07]
brain_area_res_flt = brain_area_res[pr2_matrix>0.07]
...
smModel_plot[:,brain_area_res_flt=='MST'] = np.clip(smModel_plot[:,brain_area_res_flt=='MST'],0,50)
smModel_plot[:,brain_area_res_flt=='PPC'] =  np.clip(51 + smModel_plot[:,brain_area_res_flt=='PPC'],50,100)
smModel_plot[:,brain_area_res_flt=='PFC'] =  np.clip(101 + smModel_plot[:,brain_area_res_flt=='PFC'],100,150)


smAll_plot[:,brain_area_res_flt=='MST'] = np.clip(smAll_plot[:,brain_area_res_flt=='MST'],0,50)
smAll_plot[:,brain_area_res_flt=='PPC'] =  np.clip(51+ smAll_plot[:,brain_area_res_flt=='PPC'],50,100)
smAll_plot[:,brain_area_res_flt=='PFC'] =  np.clip(101 + smAll_plot[:,brain_area_res_flt=='PFC'],100,150)

smAll_plot = smAll_plot[:,srt_pca]
smModel_plot = smModel_plot[:,srt_pca]
ba_res = brain_area_res_flt[srt_pca]
...
plt.tight_layout()  
plt.savefig('60sec_raw_vs_model.tiff')
