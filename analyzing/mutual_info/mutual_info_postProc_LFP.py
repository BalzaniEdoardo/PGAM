...
if os.path.exists('/Users/edoardo/Work/Code/GAM_code/GAM_library'):
    ...
    folder_fh = '/Volumes/WD_Edo/firefly_analysis/LFP_band/lfp_fit_results/'
    done_folder = '/Volumes/WD_Edo/firefly_analysis/LFP_band/processed_data/mutual_info_LFP'
    folder_incomplete = '/Volumes/WD_Edo/firefly_analysis/LFP_band/processed_data/mutual_info_LFP/incomplete'
    ...
    folder_fh = '/scratch/jpn5/lfp_fit/lfp_fit_results/'
...
ele_dict = {'Quigley':{'MST':12,'PPC':54,'VIP':12},
            'Schro':{'PPC':28,'PFC':76,'MST':12,'VIP':12},
            'Bruno':{'PPC':54},
            'Jimmy':{'MST':12,'PFC':12,'VIP':12},
            'Viktor': {'MST': 12, 'PFC': 12, 'VIP': 12}}
...
def compute_mutual_info(spikes_neu, sm_handler, modelX, trial_type, trial_idx, index_dict, neuron,
    ...
    mi_dtype = {
        'names': ('monkey', 'session', 'neuron', 'electrode_id', 'cluster_id', 'brain_area', 'variable',
                  'manipulation_type',
                  'manipulation_value',
                  'mutual_info', 'significance', 'pseudo-r2'),
        'formats': ('U30', 'U30', int, int, int, 'U30', 'U30', 'U30', float, float, bool, float)}
    tun_dtype = {
    ...
    for cond in cond_dict.keys():
        for value in cond_dict[cond]:
            fhName = os.path.join(folder_fh,'gam_%s'%session,
                    'LFP_fit_results_%s_c%d_%s_%.4f.dill'%(session,neuron,cond,value))
            # print(cond,value)
            ...
            with open(fhName,'rb') as fh:
                ...
                ele_id = res['electrode_id']
                clu_id = res['cluster_id']
                del res
            ...
            for var in mi.keys():
                ...
                mi_info_tmp['electrode_id'][cc] = ele_id
                mi_info_tmp['cluster_id'][cc] = clu_id
...
lfp_alpha_all = deepcopy(lfp_alpha)
lfp_beta_all = deepcopy(lfp_beta)
lfp_theta_all = deepcopy(lfp_theta)
...
assert(brain_area.shape[0]==electrode_id.shape[0])
assert(brain_area.shape[0]==cluster_id.shape[0])

all_ba = np.unique(brain_area)
first = True
for neuron in keep_unit:
    print('adding neuron',neuron)
    idx_neu = np.where(keep_unit==neuron)[0][0]

    ba_neu = brain_area[idx_neu]
    ele_id_neu = electrode_id[idx_neu]
    cluster_id_neu = cluster_id[idx_neu]
    ...
    if 'spike_hist' in sm_handler.smooths_dict.keys():
        sm_handler.smooths_dict.pop('spike_hist')
        sm_handler.smooths_var.remove('spike_hist')

    sm_handler.add_smooth('spike_hist', [x_trans], ord=order, knots=[knots],
                                          knots_num=None, perc_out_range=None,
                                  is_cyclic=[is_cyclic], lam=50,
                                  penalty_type=penalty_type,
                                  der=der,
    ...
    for var in ['lfp_beta','lfp_alpha','lfp_theta',
                'lfp_beta_MST', 'lfp_alpha_MST', 'lfp_theta_MST',
                'lfp_beta_PPC', 'lfp_alpha_PPC', 'lfp_theta_PPC',
                'lfp_beta_PFC', 'lfp_alpha_PFC', 'lfp_theta_PFC',
                'lfp_beta_VIP', 'lfp_alpha_VIP', 'lfp_theta_VIP'
                ]:

        splt = var.split('_')
        if len(splt) == 3:
            ba = splt[2]
            if not any( all_ba==ba):
                continue
            # if ba == ba_neu:
            #     continue
        ...
        if first:
            ...
            # check if cross LFP needs to be added
            if not ba is None:
                mnk = monkey_dict[session.split('s')[0]]

                ele_id = ele_dict[mnk][ba]
                sel = (ele_id_all == ele_id) & (brain_area_all == ba)
                while sel.sum() == 0:
                    ele_id = ele_id - 1
                    sel = (ele_id_all == ele_id) & (brain_area_all == ba)
                if sel.sum() > 1:
                ...
                if var == 'lfp_theta_%s'%ba:
                    x = np.squeeze(lfp_theta_all[keep, sel])

                elif var == 'lfp_beta_%s'%ba:
                    x = np.squeeze(lfp_beta_all[keep, sel])
                elif var == 'lfp_alpha_%s'%ba:
                    x = np.squeeze(lfp_alpha_all[keep, sel])

        if first or (len(splt)==2):
            knots, x_trans, include_var, is_cyclic, order,\
                kernel_len,kernel_direction,is_temporal_kernel,penalty_type,der =\
                    knots_cerate(x,'_'.join(splt[:2]),session,hand_vel_temp=hand_vel_temp,hist_filt_dur='short',
                    exclude_eye_position=['m44s213','m53s133','m53s134','m53s105','m53s182'],
                    condition=cond_knots)
    ...
    if first:
        ...
        # for ba in all_ba:
        #     tmp = sm_handler['lfp_beta_%s'%ba].additive_model_preprocessing()[0].toarray()
        #     modelX[:, index_dic_orig['lfp_beta_%s'%ba]] = tmp
        #     tmp = sm_handler['lfp_alpha_%s'%ba].additive_model_preprocessing()[0].toarray()
        #     modelX[:, index_dic_orig['lfp_alpha_%s'%ba]] = tmp
        #     tmp = sm_handler['lfp_theta_%s'%ba].additive_model_preprocessing()[0].toarray()
        #     modelX[:, index_dic_orig['lfp_theta_%s'%ba]] = tmp
    ...
    mutual_info_neu,tuning_neu = compute_mutual_info(spikes_neu, sm_handler, modelX, trial_type,
                        ...
                        folder_fh, session, cond_dict, monkey_dict, brain_area[idx_neu])
    mi_info = np.hstack((mi_info, mutual_info_neu))
    tuning_Hz = np.hstack((tuning_Hz, tuning_neu))

    if counter_save % 20 == 0:
        with open('LFP_mutual_info_and_tunHz_%s.dill' % session, 'wb') as fh:
