...
variable_group_dict = {'sensory':['rad_vel','ang_vel'],
                       'internal':['ang_target','rad_target'],
    ...
    for man_type in cond_dict.keys():  
        ...
        for ba in ['PPC','PFC','MST']:
            ...
            for session in session_list:
                for variable in variable_combine:
                    ...
                    for unit in tuned_both:
                        ...
                        # shuffle all
                        tun_shuffle = tuning_matrix_dict[variable][bool_even * (info_evals['unit id']==shuffle_id_odd[cc%(shuffle_id_odd.shape[0])]),:]
                        tun_shuffle2 = tuning_matrix_dict[variable][bool_even * (info_evals['unit id']==shuffle_id_even[cc%(shuffle_id_even.shape[0])]),:]
            ...
            brain_area_similarity[man_type][ba] = deepcopy(similarity_all)
                ...
                # plt.plot(resp[1,:])
                # break
        similarity_dict_group[variable_group] = deepcopy(brain_area_similarity)



ax_dict = {}
fig_dict = {}
ylim_joint = [np.inf,-np.inf]

color_var = {}
for ba in ['MST','PPC','PFC']:
    var_groups = list(similarity_dict_group.keys())
    brain_areas = list(similarity_dict_group[var_groups[0]][man_type].keys())
    if not ba in brain_areas:
        continue
    
    
    
    fig_dict[ba] = plt.figure(figsize=[4.73, 4.8 ])
    ax_dict[ba] = plt.subplot(1,1,1)
    plt.title('similarity '+ ba + ' ' + man_type)
    x = np.linspace(-1,1,100)
    for variable in  var_groups:
        if variable =='acceleration' and ba =='PFC':
            continue
        similarity_cond = similarity_dict_group[variable][man_type][ba]
        similarity_control = similarity_dict_group[variable]['odd'][ba]
        if similarity_cond.shape[1] < 5 or similarity_control.shape[1] < 5:
            continue
        
        ecdf_cond = ECDF(similarity_cond[0,~np.isnan(similarity_cond[0,:])])
        ecdf_control = ECDF(similarity_control[0,~np.isnan(similarity_control[0,:])])
        
        

        
        if not variable in color_var.keys():
            p, = plt.plot(x,ecdf_cond(x)-ecdf_control(x),label=variable)
            color_var[variable] = p.get_color()
        else:
            p, = plt.plot(x,ecdf_cond(x)-ecdf_control(x),label=variable,color=color_var[variable])
        ylim = plt.ylim()
        
        ylim_joint[0] = np.min([ylim_joint[0],ylim[0]])   
        ylim_joint[1] = np.max([ylim_joint[1],ylim[1]])
        plt.xlabel('similarity index')
        plt.ylabel('delta cdf')

    plt.legend()
for key in ax_dict.keys():
    ax_dict[key].set_ylim(ylim_joint)
    fig_dict[key].tight_layout()
    fig_dict[key].savefig('Figs/%s_%s_%s_compare_condition.png'%(monkey_dict[monkey],key,man_type))
# ls = ['-','--']
# cc = 0
# for man_type in cond_dict.keys():
