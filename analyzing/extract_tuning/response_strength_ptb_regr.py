...
def compute_integral_mean(gam_res,var,discr=1000):
    ...
    if gam_res.smooth_info[var]['is_temporal_kernel'] and (var == 'spike_hist' or var =='t_ptb'):
...
fhName = '/Volumes/WD_Edo/firefly_analysis/LFP_band/concatenation_with_accel/m53s39.npz'
...
npz_path = None
first=True
path_to_gam = '/Volumes/WD_Edo/firefly_analysis/LFP_band/gam_ptb_regressor/'
...
# check done already
if os.path.exists('response_strength_info_ptb_regr.npy'):
...
for (root,dirs,files) in os.walk(path_to_gam):
    ...
    for name in files:
        ...
        if session != session_prev:
            ...
            np.save('response_strength_info_ptb_regr.npy',info_matrix)
