...
man_value = info['manipulation value'][keep]
...
for n_neighbors in [10,25,50,100]:
    ...
    plt.scatter(umap_res[man_value==1,0],
                umap_res[man_value==1,1],s=10,alpha=0.8,label='%f'%1)
    
    plt.scatter(umap_res[man_value==1.5,0],
                umap_res[man_value==1.5,1],s=10,alpha=0.8,label='%f'%1.5)

    plt.scatter(umap_res[man_value==2,0],
                umap_res[man_value==2, 1],s=10,alpha=0.8,label='%f'%2)
    plt.legend()
...
# denoised_data = np.dot(np.dot(X,pca_model.components_[:optNum,:].T),pca_model.components_[:optNum,:]) + data.mean(axis=0)
# plt.figure()
# kk=1
# for label in np.unique(clusterer.labels_):
#     if label == -1:
...
# def get_significance(idx,info,var_list,bsfld='/Volumes/WD_Edo/firefly_analysis/LFP_band/cubic_gam_fit_with_coupling/'):
...
# sign_dict = get_significance(idx,info,var_list,bsfld='/Volumes/WD_Edo/firefly_analysis/LFP_band/cubic_gam_fit_with_coupling/')
