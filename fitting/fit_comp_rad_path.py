...
try:
    ...
    folder_name = ''
    sv_folder_base = ''
    fhName = os.path.join('/Volumes/WD_Edo/firefly_analysis/LFP_band/concatenation_with_accel/','m53s113.npz')
...
for neuron in neuron_list:
    data_dict = {}
    for rad_path_type in ['xy','cumsum']:
        ...
        for var in np.hstack((var_names, ['lfp_beta','lfp_alpha','lfp_theta','spike_hist'])):
            ...
            # if var !='spike_hist':
            #     continue
            if rad_path_type == 'xy' and var == 'rad_path':
                continue
            
            if rad_path_type == 'cumsum' and var == 'rad_path_from_xy':
                continue
            ...
            if var == 'lfp_theta':
                ...
                if var == 'rad_path':
                    cc2 = np.where(var_names == 'rad_path_from_xy')[0][0]
                    tmp = Xt[keep_test, cc2].squeeze()
                    set_nan = np.isnan(tmp)
                    x_test[set_nan] = np.nan
        ...
        data_dict[rad_path_type]= {
          'full':deepcopy(full_coupling),
          'reduced':deepcopy(reduced_coupling),
          'p_r2_coupling_full':deepcopy(p_r2_coupling),
          'p_r2_coupling_reduced':deepcopy(p_r2_coupling_reduced),
          'brain_area':deepcopy(brain_area[neuron-1]),
          'unit_type':deepcopy(unit_type[neuron-1]),
          'cluster_id':deepcopy(cluster_id[neuron-1]), 
          'electrode_id':deepcopy(electrode_id[neuron-1]),
          'channel_id':deepcopy(channel_id[neuron-1]),
          'all_areas':deepcopy(brain_area),
          'xlim':deepcopy(dict_xlims)
          }
        
    with open('gam_%s/rad_pathComp_%s_c%d_%s_%.4f.dill'%(session,session,neuron,cond_type,cond_value),'wb') as fh:
