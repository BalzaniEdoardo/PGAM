...
path_to_fit_fld = os.path.join(path_jpn5_base,'lfp_fit')#'/scratch/%s/fit_ptb_as_variable'%send_to
...
fhname = '/Users/edoardo/Work/Code/GAM_code/fitting/LFP_model_fit_cluster.py'
...
os.system('sshpass -p "%s" scp %s %s@greene.hpc.nyu.edu:%s' % ( PP, '/Users/edoardo/Work/Code/GAM_code/fitting/LFP_model_fit_cluster.py',send_to,path_to_fit_fld))
...
for root, dirs, files in os.walk(main_dir):
    ...
    for fhname in files:
        if 'condition_list_' in fhname:
            ...
            sh_text=sh_text.replace('gam_fit.py','LFP_model_fit_cluster.py')
            sh_text=sh_text.replace('jp_final_gam_fit_coupling','lfp_fit')
            sh_text=sh_text.replace('jpn5@nyu.edu','eb162@nyu.edu')
            sh_text = sh_text.replace('time=1-14:30:00', 'time=6:30:00')
            sh_text = sh_text.replace('mem=35GB', 'mem=16GB')
