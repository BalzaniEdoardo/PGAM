...
plt.close('all')
dat = np.load('input_hist_acc.npz')
...
for var in np.unique(info['variable']):
    ...
    plt.savefig('acc_input_cdf_%s.png'%var)
...
for var in np.unique(info['variable']):
    ...
    for kk in range(edge.shape[0]):
        plt.hlines(y=kk, 
               xmin=min_max[kk,0], 
               xmax=min_max[kk,1])
    plt.savefig('acc_input_domain_%s.png'%var)
...
for session in np.unique(info['session']):
    if kk % 20 == 0:
        if kk != 0:
            plt.tight_layout()
            plt.savefig('acc_vert_eye_pos_%d.png'%(kk//4))
...
for session in np.unique(info['session']):
    if kk % 20 == 0:
        if kk != 0:
            plt.tight_layout()
            plt.savefig('acc_hori_eye_pos_%d.png'%(kk//4))
        plt.figure(figsize=(10,8))
        sub_num = 1
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_neg[k] = np.where(sele_edges[k,:] < -350)[0][-1] 
...
print('ang acc outside (-350,350):',max(frac_neg)+max(frac_pos))
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_neg[k] = np.where(sele_edges[k,:] < -1500)[0][-1] 
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_pos[k] = np.where(sele_edges[k,:] > 1500)[0][0] 
...
print('rrad acc outside (-1500,1500):',max(frac_neg)+max(frac_pos))
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_neg[k] = np.where(sele_edges[k,:] < -100)[0][-1] 
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_pos[k] = np.where(sele_edges[k,:] > 100)[0][0] 
...
print('hand vel outside (-100,100):',max(frac_neg)+max(frac_pos))
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_neg[k] = np.where(sele_edges[k,:] < -70)[0][-1] 
...
for k in range(idx_edge_0.shape[0]):
    try:
        idx_edge_pos[k] = np.where(sele_edges[k,:] > 70)[0][0] 
...
print('hand vel outside (-70,70):',max(frac_neg)+max(frac_pos))
