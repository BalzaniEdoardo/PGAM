    ...
    for condition in cond_dict.keys():
        ...
        for value in cond_dict[condition]:
            ...
            if any(info_trial[condition]==value):
                for neu in neuron_use:
                    pattern = '^fit_results_%s_c%d_%s_%.4f.dill$'%(session,neu,condition,value)
                    check_fld = '/Volumes/WD_Edo/firefly_analysis/LFP_band/GAM_fit_with_acc/gam_%s/'%session
                    
                    add_cond = True
                    for name in os.listdir(check_fld):
                        if re.match(pattern,name):
                            add_cond = False
                            break
                    if add_cond:
                        tmp_cond_list = np.zeros(1,dtype=dict_type)
                        tmp_cond_list['neuron'] = neu
