...
path_to_fit_fld = os.path.join(path_jpn5_base,'fit_lfp_vs_coupling')#'/scratch/%s/fit_ptb_as_variable'%send_to
...
fhname = '/Users/edoardo/Work/Code/GAM_code/select_variables/with_and_wo_LFP_compare.py'
...
os.system('sshpass -p "%s" scp %s %s@greene.hpc.nyu.edu:%s' % ( PP, fhname,send_to,path_to_fit_fld))
...
for root, dirs, files in os.walk(main_dir):
    ...
    for fhname in files:
        if 'condition_list_' in fhname:
            ...
            sh_text=sh_text.replace('gam_fit.py','with_and_wo_LFP_compare.py')
            sh_text=sh_text.replace('jp_final_gam_fit_coupling','fit_lfp_vs_coupling')
            sh_text=sh_text.replace('jpn5@nyu.edu','eb162@nyu.edu')
            sh_text = sh_text.replace('time=1-14:30:00', 'time=6:30:00')
            sh_text = sh_text.replace('mem=35GB', 'mem=35GB')
