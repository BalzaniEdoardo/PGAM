...
regex = '^m[0-9]+s[0-9]+.mat$'
reggex2 = '^lfp_.+_m[0-9]+s[0-9]+.mat$'
sshPass = 'sshpass'
...
dest_path = '/scratch/eb162/dataset_firefly/'

list_session = []
for root, dirs, files in os.walk("/Volumes/WD Edo/firefly_analysis/LFP_band/DATASET"):
    for fh in files:
        if re.match(regex,fh) or re.match(reggex2,fh):
            session = fh.split('.')[0]
            send_path = '\ '.join(os.path.join(root,fh).split(' '))
            run_command(commandCopy%(send_path,dest_path))
              # make dir
