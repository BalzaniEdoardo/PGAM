...
```
docker pull  edoardobalzani87/pgam:1.2
```
...
```
docker run -ti  edoardobalzani87/pgam:1.2 /bin/bash
```
...
```
	docker run   -v your-notebook-folder:/notebooks -ti -p 8888:8888 edoardobalzani87/pgam:1.2
```
...
```
docker run -v your-script-folder/:/scripts -ti -p 8888:8888 edoardobalzani87/pgam:1.2 /bin/bash -c "python scripts/yourscript.py"
```
...
2. Open the terminal/command prompt and create a conda environment with
	...
	```
	conda create -n pgam python=3.11
	```
