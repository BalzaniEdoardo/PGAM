...
if plot_boolean:
    ...
    for dill_name in os.listdir('/Users/jean-paulnoel/Documents/Savin-Angelaki/saved/GAM_results_Ody/gam_%s/'%session):
        # load file
        
    
        if not '_all_' in dill_name:
            continue
        unit = int(dill_name.split('_all_')[0].split('c')[-1])
        with open('/Users/jean-paulnoel/Documents/Savin-Angelaki/saved/GAM_results_Ody/gam_%s/gam_fit_%s_c%d_all_1.0000.dill'%(session,session,unit),'rb') as fh:
