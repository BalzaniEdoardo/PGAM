...
fit_cntrl1 = gam_model.optim_gam(sm_handler.smooths_var,max_iter=10**3,tol=1e-8,conv_criteria='gcv',
...
fit_cntrl2 = gam_model.optim_gam(sm_handler.smooths_var,max_iter=10**3,tol=1e-8,conv_criteria='gcv',
                  perform_PQL=perform_PQL,use_dgcv=True,initial_smooths_guess=False,method='L-BFGS-B',
                  compute_AIC=False,random_init=False,bounds_rho=None,gcv_sel_tol=1e-10,fit_initial_beta=True,
                  filter_trials=None,compute_MI=False)
...
fX_third_400, fX_p_ci_third_400, fX_m_ci_third_400 = fit_cntrl2.smooth_compute([xx_400], 'rad_vel', perc=0.99)
fX_half_200, fX_p_ci_half_200, fX_m_ci_half_200 = fit_cntrl1.smooth_compute([xx_200], 'rad_vel', perc=0.99)



plt.figure()

p,=plt.plot(xx_400,fX_third_400,label='gain=2')
plt.fill_between(xx_400, fX_m_ci_third_400, fX_p_ci_third_400, color=p.get_color(), alpha=0.4)


p,=plt.plot(xx_200,fX_half_200,label='gain=1')
plt.fill_between(xx_200, fX_m_ci_half_200, fX_p_ci_half_200, color=p.get_color(), alpha=0.4)

plt.legend()

plt.tight_layout()
plt.savefig('/Users/edoardo/Work/Code/GAM_code/plotting/Figs/check_controlgain_%s_%d.png'%(session,neuron))
