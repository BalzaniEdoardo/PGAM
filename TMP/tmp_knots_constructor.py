...
if __name__ == '__main__':
    ...
    # info = dat['info']
    use_var = 'ang_vel'
    test_tuning =  lambda x : x/100.#np.exp(-(x-100)**2/50**2.)
    ...
    for var in np.hstack((var_names, ['lfp_beta','spike_hist'])):
        ...
        if var == 'lfp_theta':
            ...
            true_meanRate = yt[:,neuron-1].mean()
            mu_spk = np.exp(test_tuning(x))
            mu_spk = mu_spk / np.nanmean(mu_spk) * true_meanRate
            spk_test = np.random.poisson(mu_spk)
    ...
    gam_model = general_additive_model(sm_handler, sm_handler.smooths_var, spk_test, poissFam,
                                        fisher_scoring=True)
...
# # =============================================================================
    ...
    for var in np.hstack((var_names, ['lfp_beta','spike_hist'])):
        ...
        if var == 'spike_hist':
            ...
            ff = test_tuning(xx)-np.log(mu_spk.mean()) + np.log(true_meanRate)
            ff  = ff - (ff[0]  - fX[0])
            plt.plot(xx,ff)
    ...
    for k in range(numbasis):
        ...
        fX,_,_ = full_tmp.smooth_compute([xx],use_var)
        plt.plot(xx,fX)    