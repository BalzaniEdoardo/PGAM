...
# if False:
    ...
    if reload:
        (Xt, yt, lfp_beta, lfp_alpha, lfp_theta, var_names, trial_type,
         trial_idx_all, brain_area, pre_trial_dur, pre_trial_dur, time_bin,
    ...
    unit_list = np.arange(yt.shape[1])+1
    unit_list = unit_list[combine_filter]
    
    fX_matrix = np.zeros((unit_list.shape[0],100))
    fX_mci_matrix = np.zeros((unit_list.shape[0],100))
    fX_pci_matrix = np.zeros((unit_list.shape[0],100))
    nnn = 0
    for neuron in unit_list:
        print('NEURON',neuron)
        if cond_type != 'odd':
        ...
        for ii in idx_subselect:
            keep = np.hstack((keep, np.where(trial_idx_all == ii)[0]))
    
        print(' condition', cond_type, cond_value)
    
        keep = np.array(keep, dtype=int)
        trial_idx = trial_idx_all[keep]
    ...
    # # =============================================================================
        ...
        for var in np.hstack((var_names, ['lfp_beta','spike_hist'])):
            ...
            if gam_res.smooth_info[var]['is_temporal_kernel']:
                ...
                if var == use_var:
                    fX_mci_matrix[nnn,:] = fX_m_ci
                    fX_pci_matrix[nnn,:] = fX_p_ci
                    fX_matrix[nnn, :] = fX
                    xx_var = deepcopy(xx)
                    nnn += 1
    ...
    np.savez('/Users/edoardo/Work/Code/GAM_code/TMP/rad_path_comp/%s_checkTuning.npz'%use_var,x=xx_var,fX_matrix=fX_matrix,
              fX_pci_matrix=fX_pci_matrix,fX_mci_matrix=fX_mci_matrix)
