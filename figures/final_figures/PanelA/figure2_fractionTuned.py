...
# split monkey
monk_list = ['Schro','Quigley']
sign_mat = np.zeros(len(monk_list)*4,dtype={'names':['monkey','brain_area']+var_order,
                             'formats':['U30','U30']+[float]*len(var_order)})


mnk_num = 0
for monkey in monk_list:
    sel1 = mutual_info['monkey'] == monkey
    
    for ba in ['MST','PPC','PFC','VIP']:
        sel2 = mutual_info['brain_area'] == ba
        sign_mat['monkey'][mnk_num] = monkey
        sign_mat['brain_area'][mnk_num] = ba
        for var in var_order:
            
            sel3 = (mutual_info['variable'] == var) & sel1 & sel2
...
# pool monkey
sign_mat_pooled = np.zeros(4,dtype={'names':['brain_area']+var_order,
                             'formats':['U30']+[float]*len(var_order)})
...
for ba in ['MST','PPC','PFC','VIP']:
    ...
    for var in var_order:
        ...
        sel3 = (mutual_info['variable'] == var) & sel2
...
for ba in ['MST','PPC','PFC']:
    ...
    plt.plot(x_ticks+ccba*dx_ba, y_axis,ls='none', marker='^',ms=8,color=color_dict[ba])
    ccba+=1
...
plt.xlim(xlim)
plt.tight_layout()
# plt.savefig('/Volumes/WD_Edo/firefly_analysis/LFP_band/FINALFIG/Figure2/raw_pdf/frac_tuned.pdf')
